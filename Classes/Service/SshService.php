...
class SshService {
	...
	protected function connectToRemoteServerFTP() {
		...
		if (!$login) {
			throw new \TYPO3\CMS\Extbase\Mvc\Exception('Unable to establish sftp connection! ' . print_r($sftpConnection->getErrors(), TRUE), 1324560948);
		} else {
	...
	protected function connectToRemoteServerSSH2() {
		...
		if (!$login) {
			throw new \TYPO3\CMS\Extbase\Mvc\Exception('Unable to connect to remote server for SSH2!' . print_r($sshConnection->getErrors(), TRUE), 1324560948);
		} else {
